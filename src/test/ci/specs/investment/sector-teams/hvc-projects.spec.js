...
describe( 'Investment Sector Team HVC project Page', function(){

	beforeAll( function( done ){

		fetch( '/investment/sector-teams/1/hvc-projects/' ).then( takeScreenshot( 'investment_sector_team_hvc_projects' ) ).then( done );
	} );

	accessibilityCheck( 'investment_sector_team_hvc_projects' );
	...
	describe( 'Psge tabs', function(){
		...
		it( 'Should have the correct tab active on the page', function( done ){
			...
			tab[ 0 ].getText().then( ( text ) => {
				...
				expect( text ).toEqual( 'HVC projects' );
				done();
			} );
		} );
	} );
