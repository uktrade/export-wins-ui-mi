...
describe( 'FDI Sector Team markets', function(){
	...
	describe( 'With a sector team overview', function(){

		it( 'Should use the description for the name', function(){

			const input = getBackendStub( '/investment/fdi/sector_teams/overview' );
			const teams = input.results.sector_teams;
			const output = marketsViewModel.create( teams );

			output.forEach( ( market, index ) => {

				expect( market.name ).toEqual( teams[ index ].description );
			} );
		} );
	} );
} );
